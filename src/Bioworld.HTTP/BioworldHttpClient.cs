namespace Bioworld.HTTP
{
    ...
    public class BioworldHttpClient : IHttpClient
    {
        ...
        protected virtual Task<HttpResponseMessage> SendAsync(string uri, Method method, HttpContent content = null) =>
            Policy.Handle<Exception>()
                ...
                .ExecuteAsync(() =>
                {
                    var requestUri = uri.StartsWith("http") ? uri : $"https://{uri}";
        ...
        protected StringContent GetJsonPayload(object data)
        {
            ...
            var content = new StringContent(JsonSerializer.Serialize(data, SerializerOptions), Encoding.UTF8,
                "application/json");
